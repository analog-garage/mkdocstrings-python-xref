...
def test_substitute_relative_crossref(caplog: pytest.LogCaptureFixture) -> None:
    ...
    def assert_sub(parent: Object, text: str, ref: str, expected: str = "", warning: str = "") -> None:
        if not expected:
            expected = ref
        crossref = f"[{text}][{ref}]"
        doc = Docstring(parent=parent, value=crossref, lineno=42)
